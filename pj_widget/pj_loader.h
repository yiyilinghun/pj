...
struct WPixel
{
    union { WORD m_Color/*16bit*/; struct { WORD Blue : 5/*5bit*/; WORD Green : 6/*6bit*/; WORD Red : 5/*5bit*/; }S_TEMP; }; uint32_t MakeARGB(uint32_t A) { uint32_t R = (m_Color << 8 & 0xf80000); uint32_t G = (m_Color << 5 & 0x7fc00); uint32_t B = ((m_Color << 3 | (m_Color & 7)) & 0x3ff); uint32_t dwColor = (A << 24) + R + G + B; return dwColor; }; operator uint32_t() const { return (m_Color << 5 & 0x7fc00) | (m_Color << 8 & 0xf80000) | ((m_Color << 3 | (m_Color & 7)) & 0x3ff); }
};
...
public:
    // 加载资源文件
    Boolean pjLoadFile(mstr filename, uint32_t filekey);
    ...
    // 加载动画
    Boolean pjGetWasTextures(xyTextureInfo& textureInfo, std::vector<MemoryStream>& xListTexStream);

    // 资源map
    std::map<uint64_t, xyUnit> m_mapReses;

    // 资源文件key,文件路径名对应map
    std::map<uint32_t, mstr> m_mapReskey_ResFilename;
